...
The \`{{ ... }}\` is **Jinja**, a templating language. A dbt model file is SQL with bits of Jinja mixed in. Before running anything, dbt **compiles** the Jinja away: \`{{ ref('stg_customers') }}\` becomes the real table name in your warehouse. You can see the compiled SQL anytime with \`dbt compile\`.
...
`,
  ...
  tasks: [
    ...
    },
    {
      ...
      validate: (s) => hasModel(s, 'customer_emails') && modelRefs(s, 'customer_emails', 'stg_customers'),
    },
    {
      id: 'lineage',
